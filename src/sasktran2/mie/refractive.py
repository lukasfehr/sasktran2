...
def _from_aria_file(key: str):
    ...
    if not (data_dir / "data_files").is_dir():
        import shutil
        shutil.unpack_archive(zippath, data_dir)
    ...
    with path.open() as f:
        ...
        t = [
            x.strip() for x in t
        ]  # strips whitespace from beginning and end of lines
        ...
        for i in range(header_lines):
            ...
            if line[0] != "#":
                ...
                if tag_name not in expected_header_names:
                    print(f'Unknown header tag "{tag_name}", so ignored (file: {path})')
                    continue
        ...
        for cl in column_labels:
            if cl not in expected_column_names:
                print(f'Unknown column name "{cl}", so ignored (file: {path})')
                continue
            out["data"][cl] = []
    ...
    return interpolate.interp1d(
        1e3 * wavl[isort], n[isort] - 1j * k[isort]
    )
...
class Water(RefractiveIndex):
    def __init__(self, source: str = "osiris") -> None:
        ...
        if source.lower() == "osiris":
            ...
            super().__init__(
                _from_aria_file("H2O_Mcgarragh_2018"),
                "water_aria"
            )
        else:
